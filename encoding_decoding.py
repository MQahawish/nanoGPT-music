...
def all_midi_to_text(midi_directory, text_directory):
    # iterate over all midi files in directory and save text representation
    for filename in os.listdir(midi_directory):
        if filename.endswith(".mid"):
            path=os.path.join(midi_directory, filename)
            #check if there is already a text file with the same name
            if os.path.isfile(f"{text_directory}/{filename.split('.')[0]}.txt"):
                print(f"Text file {filename.split('.')[0]}.txt already exists.")
                continue
            save_text_representation(path, text_directory)
            continue
        else:
            continue

def save_text_representation(path, output_dir):
    ...
    # Take the name of the MIDI
    name = path.split("\\")[-1].split(".")[0]
    
    # Save this to a file
    try:
        with open(f"{output_dir}/{name}.txt", 'w') as f:
            f.write(midi2txt(path))
...
def midi2txt(path):
    ...
    # return the list as a string but add "START " and " END" to the beginning and end
    result.insert(0, "START")
    return " ".join(result)

# process midi from path
...
def check_tuple(tup):
    first, second = tup
    if first.startswith("sepxx") or first.startswith("n"):
...
def process_text(text):
    ...
    words = text.split()
    tuples = [[words[i], words[i+1]] for i in range(0, len(words), 2)]

    # Filter out unwanted tuples
    tuples = [tup for tup in tuples if check_tuple(tup)]

    for tup in tuples:
        if tup[0] == "sepxx" and tup[1].startswith("d"):
            if int(tup[1][1:]) > 5:
                tup[1] = "d5"
    ...
    for tup in tuples:
        try:
            if tup[0] == "sepxx":
                int_tuples.append((-1, int(tup[1][1:])))
            else:
                int_tuples.append((int(tup[0][1:]), int(tup[1][1:])))
        except Exception as e:
            print(f"An error occurred while converting to int: {e}")
            return None
